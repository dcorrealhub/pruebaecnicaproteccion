...
- **Despliegue — Railway (backend) y Vercel (frontend):** La BD ya corre en Railway. La decisión de usar Railway y Vercel fue por agilidad — en el contexto de una prueba técnica permiten desplegar en minutos sin configuración de infraestructura. En un entorno corporativo real, especialmente en una entidad regulada por la SFC, estas plataformas no serían la primera opción para módulos en el centro del negocio: se iría por IaC (Terraform, Pulumi) sobre infraestructura propia o cloud privada, con control a bajo nivel sobre red, seguridad y cumplimiento. Railway y Vercel son válidos para productos que requieren agilidad o que no manejan datos sensibles directamente.

- **SonarQube y cobertura mínima definida:** El pipeline de CI debe tener un quality gate configurado que bloquee merges si no se cumple el umbral de cobertura — eso está en manos del equipo de infraestructura, no del desarrollador. Lo que sí está en control del desarrollador es usar SonarLint en el IDE o el SonarQube del entorno de desarrollo para detectar problemas antes de subir el código. El umbral mínimo lo define el equipo junto con el área de calidad, referencia estándar 80% para módulos críticos.
